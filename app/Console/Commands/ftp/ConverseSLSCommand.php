...
class ConverseSLSCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ftp:converse';
     ...
     * @var string
     */
    protected $description = 'Envia Venta de Converse';
    ...
    public function ftpFunctionSLS($fechaInicio, $fechaFin, $horaActual)
    {
        ...
        try {
            ...
            if (!$connection) {
                Log::channel('converse')->error('SAP S4 connection error.');
            }
            ...
            $condiciones[] = "a.matkl in  ('CV','UM','FI')";
            ...
            //$directory = "/home/sftp/report/converse/";

            if (empty($content) || empty($filename)) {
                Log::channel('converse')->error('El contenido o el nombre del archivo está vacío. No se envió nada al FTP.');
            }
            try {
                $result = Storage::disk('converse')->put($filename, $content);

                if ($result) {
                    // Log::info('Archivo ' . $filename . ' almacenado con éxito en el FTP en el directorio ' . $directory);
                } else {
                    Log::channel('converse')->error('Hubo un problema al guardar el archivo ' . $filename . ' en el FTP en el directorio ');
                }
            } catch (\Exception $e) {
                Log::channel('converse')->error('Error al intentar guardar en el FTP: ' . $e->getMessage());
            }
        } catch (\Exception $e) {
            Log::channel('converse')->error($e->getMessage());
        }
    }
