...
class GetYearSalesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:sales:year';
    ...
    public function handle()
    {
                        ...
                        if ($channel === '01') {
                            ...
                            if ($results) {
                                while ($row = odbc_fetch_array($results)) {
                                    ...
                                    // Log::info($banner['bukrs'] .'-'. $banner['vkorg'] .'-'. $channel .'-'. $date .'-'. $row['NAME1'] .'-'. intval($row['TOTAL']));
                                }
                            }
                            else {
                                Log::error('SAP S4 query error: ' . $query);
                            }
                        }
                        else {
        ...
        Log::info('GET SAP S4 Sales Month: ' . $executionTime . ' seconds');
