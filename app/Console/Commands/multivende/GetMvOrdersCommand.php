...
class GetMvOrdersCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:mvorders';
     ...
     * @var string
     */
    protected $description = 'Get Multivende Orders in Processing Status';
    ...
    public function handle()
    {
        ...
        $startDate = $startTime->copy()->subDays(1)->format('Y-m-d');
        $endDate = $startTime->copy()->addDays(1)->format('Y-m-d');

        try {
            // Llamar al comando get:mvaccess desde dentro de este comando
            $exitCode = Artisan::call('get:mvaccess');
            if ($exitCode === 0) {
                $stores = MvStore::where('active', 1)
                ...
                foreach ($stores as $store) {
                    ...
                    $this->getCheckOutsLight($access->base_url, $access->merchant_id, $access->token, $account, $startDate, $endDate, $store->marketplace_id, $store->id, $store->connection);
                }
            } else {
    ...
    private function getCheckOutsLight($base_url, $merchant_id, $token, $account, $startDate, $endDate, $marketplace_id, $store_id, $connection)
    {
        if (!empty($account)) {
            try {
                ...
                for ($i = 1; $i <= $totalPages; $i++) {
                    ...
                    foreach ($orders['entries'] as $entry) {
                        ...
                        if ($result->isEmpty()) {
                            $this->getCheckOut($checkout_id, $marketplace_id, $base_url, $token, $store_id, $connection);
                        }
                    }
                }
            } catch (\Exception $e) {
    ...
    private function getCheckOut($checkout_id, $marketplace_id, $base_url, $token, $store_id, $connection)
    {
        ...
        try {
            ...
            //if (is_null($order)) {
            //if ($externalCustomerOrderNumber == '2000004941533773') {
