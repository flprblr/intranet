...
class PostPosCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'Post E-Commerce Orders to Pos';
    ...
    private function prepareFrame($ecommerce, $company, $order, $payment)
    {
        $separator = "\n";
        $frame = "<arg0>
                        <tipo_documento>101</tipo_documento>
                        ...
                        <nro_documento>" . $order->invoice_number . "</nro_documento>
    ...
    private function sendToPos($integration_mode, $company,  $xml, $order, $ecommerce)
    {
        ...
        $this->changeStatus($order->id, 6, $ecommerce); // POS: Enviado

        $response = $response->getBody()->getContents();
        $response = $this->everything_in_tags($response);

        if ($response === 'OK') {
            $this->changeStatus($order->id, 8, $ecommerce); // POS: Emitido
        } else {
            if ($this->checkInvoice($company, $order->invoice_number)) {
                $this->changeStatus($order->id, 8, $ecommerce); // POS: Emitido
            } else {
                $this->changeStatus($order->id, 7, $ecommerce); // POS: Error
            }
        }
    }
