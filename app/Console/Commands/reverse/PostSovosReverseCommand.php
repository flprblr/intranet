...
use App\Models\Sovos;

use GuzzleHttp\Client;
use Automattic\WooCommerce\Client as WooClient;
use Illuminate\Support\Facades\Config;

class PostSovosReverseCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'post:sovosReverse';
     ...
     * @var string
     */
    protected $description = 'Post E-Commerce Orders Reverse to Sovos';
    ...
    public function handle()
    {
            ...
            foreach ($ecommerces as $ecommerce) {
                ...
                $orders = Order::select('id', 'order_number', 'invoice_number', 'date_created', 'net', 'tax', 'total', 'billing_first_name', 'billing_last_name', 'billing_rut', 'billing_phone', 'billing_email', 'billing_state', 'billing_city', 'billing_address_1', 'order_status_id')
                    ->where(function ($query) {
                        $query->where('order_status_id', 20); // Sovos: Reverse
                    })
        ...
        Log::info('Post Orders Reverse ->Sovos: ' . $executionTime . ' seconds');
    ...
    private function checkOrder($company, $order, $ecommerce)
    {
        ...
        foreach ($orderDetails as $orderLine) {
            ...
            if ($count === 0) {
                $this->changeStatus($order->id, 15, $ecommerce); // POS: Error Producto
                break;
                return false;
            }
        }
    ...
    {
        $folio = $this->getFolioSovos();
        $separator = "\n";
        $frame = "FH|61|$folio|$order->date_created||||2|$order->date_created|$company->rut|$ecommerce->logo|$company->activity|$company->acteco|||$company->address|$company->commune|$company->city||$order->billing_rut|$order->billing_first_name $order->billing_last_name||$order->billing_address_1|$order->billing_city|$order->billing_state||||$order->net|0|19.00|$order->tax|$order->total|||||";
        $frame .= $separator;
        ...
        $frame .= "RE|1|39|$order->invoice_number|$order->date_created|3";
        $frame .= $separator;
        $frame .= "PE|$order->order_number||||$order->billing_phone|$ecommerce->url";
        return $frame;
    }
    ...
    private function OnlineGenerationDte($integration_mode, $xml, $orderId, $ecommerce)
    {
        ...
        try {
            ...
            $this->changeStatus($orderId, 22, $ecommerce); // Sovos: Enviado
            ...
            if ($code === '0') {
                ...
                $order->invoice_number_rev = $message;
                $order->save();

                $this->updateFolioSovos($order->invoice_number_rev);

                $this->changeStatus($orderId, 21, $ecommerce); // Sovos: Emitido
            } else {
                $this->changeStatus($orderId, 23, $ecommerce); // Sovos: Error
                Log::info($xml);
            }
        } catch (\Exception $e) {
            Log::error($e->getMessage());
    ...
    private function getFolioSovos()
    {
        $sovos = Sovos::select()->first();
        $currentFolio = $sovos->folio;
        $order = Order::select()->where('invoice_number_rev', $currentFolio)->first();

        if (!$order) {
            if ($sovos->folio > $sovos->folio_final) {
                return null;
            } else {
                return $sovos->folio;
            }
    ...
    private function updateFolioSovos($folio)
    {
        $sovos = Sovos::select()->first();
        $sovos->folio = $folio + 1;
        $sovos->save();
