...
class GetDaySalesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:sales:day';
    ...
    public function handle()
    {
        ...
        try {
            ...
            if (!$conn) {
                Log::channel('sales-daily')->error('SAP S4 connection error.');
            }
                        ...
                        if ($channel === '01') {
                            ...
                            if ($results) {
                                while ($row = odbc_fetch_array($results)) {
                                    ...
                                    // Log::channel('sales-daily')->info($banner['bukrs'] .'-'. $banner['vkorg'] .'-'. $channel .'-'. $date .'-'. $row['NAME1'] .'-'. intval($row['TOTAL']));
                                }
                            }
                            else {
                                Log::channel('sales-daily')->error('SAP S4 query error: ' . $query);
                            }
                        }
                        else {
            ...
            Log::channel('sales-daily')->error($e->getMessage());
        }

        $endTime = Carbon::now();
        $executionTime = $endTime->diffInSeconds($startTime);
        Log::channel('sales-daily')->info('GET SAP S4 Sales Day: ' . $executionTime . ' seconds');
