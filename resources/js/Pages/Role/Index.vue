<script setup>
    ...
    function destroy(name, id) {
        if (confirm("Estas seguro que deseas eliminar el rol " + name + "?")) {
    ...
    <AppLayout title="Roles">
        <template #header>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <h2 class="mt-1 font-semibold text-xl text-gray-800 float-left">Roles</h2>
                </div>
                <div class="float-right">
                    <Link
                        ...
                        :href="route('role.create')">Crear</Link>
                </div>
            </div>
        </template>
        <div class="py-8">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg">
                    ...
                    <table v-if="roles.total!=0" class="w-full text-sm text-left text-gray-900">
                        ...
                        <tbody>
                            <tr class="bg-white border-b border-gray-300 hover:bg-gray-50" v-for="role in roles.data">
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <Link :href="route('role.show', role.id)">{{ role.id }}</Link>
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ role.name }}
                                </td>
                                ...
                                <td scope="row"
                                    ...
                                    <Link
                                        ...
                                        @click="destroy(role.name, role.id)">
                                    Eliminar
                                    </Link>
                                </td>
                            </tr>
                    ...
                    <div>
                        <div class="bg-white px-4 py-4 flex items-center justify-between sm:px-6">
                            ...
                            <div class="flex-1 flex justify-between sm:hidden">
                                <div v-if="roles.links.length > 3" v-for="(link, index) in roles.links">
                                    ...
                                    <Link v-else-if="index ==roles.links.length-1" class="uppercase px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-900 bg-white hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                </div>
                            </div>
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p v-if="roles.total==0" class="uppercase text-xs text-gray-900">No se encontraron resultados</p>
                                    <p v-else class="uppercase text-xs text-gray-900">Mostrando {{ roles.from }} al {{ roles.to }} de {{ roles.total }} resultados</p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md">
                                        <div v-if="roles.links.length > 3" v-for="(link, index) in roles.links">
                                            ...
                                            <Link v-else-if="index ==roles.links.length-1" class="uppercase relative inline-flex items-center px-2 py-1 rounded-r-md border-r border-t border-b border-gray-300 bg-white text-xs font-medium text-gray-900 hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index ==roles.links.length-2" class="uppercase bg-white border-gray-300 text-gray-900 hover:bg-gray-50 hidden md:inline-flex relative items-center px-3 py-1 border text-xs font-medium" v-html="link.label" :href="link.url" />
