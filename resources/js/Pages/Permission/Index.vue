<script setup>
    ...
    function destroy(name, id) {
        if (confirm("Estas seguro que deseas eliminar el permiso " + name + "?")) {
    ...
    <AppLayout title="Permisos">
        <template #header>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <h2 class="mt-1 font-semibold text-xl text-gray-800 float-left">Permisos</h2>
                </div>
                <div class="float-right">
                    <Link
                        ...
                        :href="route('permission.create')">Crear</Link>
                </div>
            </div>
        </template>
        <div class="py-8">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg">
                    ...
                    <table v-if="dbPermissions.total!=0" class="w-full text-sm text-left text-gray-900">
                        ...
                        <tbody>
                            <tr class="bg-white border-b border-gray-300 hover:bg-gray-50" v-for="dbPermission in dbPermissions.data">
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <Link :href="route('permission.show', dbPermission.id)">{{ dbPermission.id }}</Link>
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ dbPermission.name }}
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ dbPermission.created_at }}
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ dbPermission.updated_at }}
                                </td>
                                <td scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap float-right">
                                    <Link :href="route('permission.edit', dbPermission.id)"
                                    ...
                                    <Link
                                        ...
                                        @click="destroy(dbPermission.name, dbPermission.id)">
                                    Eliminar
                                    </Link>
                                </td>
                            </tr>
                    ...
                    <div>
                        <div class="bg-white px-4 py-4 flex items-center justify-between sm:px-6">
                            <div class="flex-1 flex justify-between sm:hidden">
                                <div v-if="dbPermissions.links.length > 3" v-for="(link, index) in dbPermissions.links">
                                    ...
                                    <Link v-else-if="index ==dbPermissions.links.length-1" class="uppercase px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-900 bg-white hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                </div>
                            </div>
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p v-if="dbPermissions.total==0" class="uppercase text-xs text-gray-900">No se encontraron resultados</p>
                                    <p v-else class="uppercase text-xs text-gray-900">Mostrando {{ dbPermissions.from }} al {{ dbPermissions.to }} de {{ dbPermissions.total }} resultados</p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md">
                                        <div v-if="dbPermissions.links.length > 3" v-for="(link, index) in dbPermissions.links">
                                            <Link v-if="index==0" class="focus:z-20 uppercase relative inline-flex items-center px-2 py-1 rounded-l-md border-l border-t border-b border-gray-300 bg-white text-xs font-medium text-gray-900 hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index ==dbPermissions.links.length-1" class="uppercase relative inline-flex items-center px-2 py-1 rounded-r-md border-r border-t border-b border-gray-300 bg-white text-xs font-medium text-gray-900 hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index ==dbPermissions.links.length-2" class="uppercase bg-white border-gray-300 text-gray-900 hover:bg-gray-50 hidden md:inline-flex relative items-center px-3 py-1 border text-xs font-medium" v-html="link.label" :href="link.url" />
