    ...
    <AppLayout title="Pedidos Marketplace">
        <template #header>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <h2 class="mt-1 font-semibold text-xl text-gray-800 float-left">Pedidos Marketplace</h2>
                </div>
                <div>
                    <Link
                        ...
                        v-if="can('mvorder.create')" :href="route('mvorder.create')">Sincronizar</Link>
                </div>
            </div>
        </template>
        <div class="py-8">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg">
                    ...
                    <table v-if="mvOrders.total != 0" class="w-full text-sm text-left text-gray-900">
                        ...
                        <tbody>
                            <tr class="bg-white border-b border-gray-300 hover:bg-gray-50" v-for="order in mvOrders.data">
                                ...
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <a :href="`/mvorder/${order.id}`" type="button"
                                ...
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{
                                    order.name }}</td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{
                                    order.billing_name }} {{ order.billing_last_name }}</td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{
                    ...
                    <div>
                        <div class="bg-white px-4 py-4 flex items-center justify-between sm:px-6">
                            ...
                            <div class="flex-1 flex justify-between sm:hidden">
                                <div v-if="mvOrders.links.length > 3" v-for="(link, index) in mvOrders.links">
                                    <Link v-if="index == 0"
                                    ...
                                    <Link v-else-if="index == mvOrders.links.length - 1"
                                        class="uppercase px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-900 bg-white hover:bg-gray-50"
                                        v-html="link.label" :href="link.url" />
                                </div>
                            </div>
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p v-if="mvOrders.total == 0" class="uppercase text-xs text-gray-900">No se encontraron
                                        resultados</p>
                                    <p v-else class="uppercase text-xs text-gray-900">Mostrando {{ mvOrders.from }} al {{
                                        mvOrders.to }} de {{ mvOrders.total }} resultados</p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md">
                                        <div v-if="mvOrders.links.length > 3" v-for="(link, index) in mvOrders.links">
                                            ...
                                            <Link v-else-if="index == mvOrders.links.length - 1"
                                                ...
                                                v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index == mvOrders.links.length - 2"
