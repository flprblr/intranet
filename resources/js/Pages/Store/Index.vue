<script setup>
    ...
    function destroy(name, id) {
        if (confirm("Estas seguro que deseas eliminar la tienda " + name + "?")) {
            form.delete(route('store.destroy', id));
        }
    }
    ...
    <AppLayout title="Tiendas">
        <template #header>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <h2 class="mt-1 font-semibold text-xl text-gray-800 float-left">Tiendas</h2>
                </div>
                <div>
                    <Link
                        ...
                        v-if="can('store.create')" :href="route('store.create')">Crear</Link>
                </div>
            </div>
        </template>
        <div class="py-8">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg">
                    ...
                    <table v-if="stores.total!=0" class="w-full text-sm text-left text-gray-900">
                        <thead class="text-xs text-gray-900 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3">N° Tienda</th>
                                <th scope="col" class="px-6 py-3">Descripción</th>
                                <th scope="col" class="px-6 py-3">Anexo</th>
                                <th scope="col" class="px-6 py-3">Dirección</th>
                                <th scope="col" class="px-6 py-3">Teléfono</th>
                                <th v-if="can('store.create')" scope="col" class="px-6 py-3">Contraseña Wi-Fi</th>
                                <th scope="col" class="px-6 py-3">Email</th>
                                <th scope="col" class="px-6 py-3">Estado</th>
                                <th v-if="can('store.create')" scope="col" class="px-6 py-3 float-right">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="bg-white border-b border-gray-300 hover:bg-gray-50" v-for="store in stores.data">
                                ...
                                <td v-if="can('store.create')" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ store.wifi_password }}
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ store.email}}
                                </td>
                                <td v-if="store.condition == 1" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    Abierta
                                </td>
                                <td v-else="store.condition == 0" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    Cerrada
                                </td>
                                <td v-if="can('store.create')" scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap float-right">
                                    <Link :href="route('store.edit', store.id)"
                                    ...
                                    <Link
                                        ...
                                        @click="destroy(store.name, store.id)">
                                    Eliminar
                                    </Link>
                                </td>
                            </tr>
                    ...
                    <div>
                        <div class="bg-white px-4 py-4 flex items-center justify-between sm:px-6">
                            ...
                            <div class="flex-1 flex justify-between sm:hidden">
                                <div v-if="stores.links.length > 3" v-for="(link, index) in stores.links">
                                    ...
                                    <Link v-else-if="index ==stores.links.length-1" class="uppercase px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-900 bg-white hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                </div>
                            </div>
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p v-if="stores.total==0" class="uppercase text-xs text-gray-900">No se encontraron resultados</p>
                                    <p v-else class="uppercase text-xs text-gray-900">Mostrando {{ stores.from }} al {{ stores.to }} de {{ stores.total }} resultados</p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md">
                                        <div v-if="stores.links.length > 3" v-for="(link, index) in stores.links">
                                            ...
                                            <Link v-else-if="index ==stores.links.length-1" class="uppercase relative inline-flex items-center px-2 py-1 rounded-r-md border-r border-t border-b border-gray-300 bg-white text-xs font-medium text-gray-900 hover:bg-gray-50" v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index ==stores.links.length-2" class="uppercase bg-white border-gray-300 text-gray-900 hover:bg-gray-50 hidden md:inline-flex relative items-center px-3 py-1 border text-xs font-medium" v-html="link.label" :href="link.url" />
