...
function destroy(name, id) {
    if (confirm("Estas seguro que deseas eliminar la empresa " + name + "?")) {
    ...
    <AppLayout title="Empresas">
        <template #header>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <h2 class="mt-1 font-semibold text-xl text-gray-800 float-left">Empresas</h2>
                </div>
                <div class="float-right">
                    <Link
                        ...
                        :href="route('company.create')">Crear</Link>
                </div>
            </div>
        </template>
        <div class="py-8">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg">
                    ...
                    <table v-if="companies.total != 0" class="w-full text-sm text-left text-gray-900">
                        <thead class="text-xs text-gray-900 uppercase bg-gray-50">
                            <tr>
                                ...
                                <!-- <th scope="col" class="px-6 py-3">Giro</th> -->
                        ...
                        <tbody>
                            <tr class="bg-white border-b border-gray-300 hover:bg-gray-50"
                                ...
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <Link :href="route('company.show', company.id)">{{ company.id }}</Link>
                                </td>
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    {{ company.name }}
                                </td>
                                ...
                                <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap float-right">
                                    ...
                                    <Link
                                        ...
                                        @click="destroy(company.name, company.id)">
                                    Eliminar
                                    </Link>
                                </td>
                            </tr>
                    ...
                    <div>
                        <div class="bg-white px-4 py-4 flex items-center justify-between sm:px-6">
                            <div class="flex-1 flex justify-between sm:hidden">
                                <div v-if="companies.links.length > 3" v-for="(link, index) in companies.links">
                                    <Link v-if="index == 0"
                                    ...
                                    <Link v-else-if="index == companies.links.length - 1"
                                        class="uppercase px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-900 bg-white hover:bg-gray-50"
                                        v-html="link.label" :href="link.url" />
                                </div>
                            </div>
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p v-if="companies.total == 0" class="uppercase text-xs text-gray-900">No se encontraron
                                        resultados</p>
                                    <p v-else class="uppercase text-xs text-gray-900">Mostrando {{ companies.from }} al {{
                                        companies.to }} de {{ companies.total }} resultados</p>
                                </div>
                                <div>
                                    <nav class="relative z-0 inline-flex rounded-md">
                                        <div v-if="companies.links.length > 3" v-for="(link, index) in companies.links">
                                            ...
                                            <Link v-else-if="index == companies.links.length - 1"
                                                ...
                                                v-html="link.label" :href="link.url" />
                                            <Link v-else-if="index ==companies.links.length-2"
